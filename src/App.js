...
export default function App() {
  ...
  if (loading) return (
    <div style={{ ...S.container, alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ color: THEME.primary, fontSize: '16px' }}>⏳ 로딩 중...</p>
    </div>
  )
  ...
  return (
    <div style={{ ...S.container, alignItems: 'center', justifyContent: 'center' }}>
      {mode === 'select' && (
        <div style={S.loginCard}>
          <div style={{ textAlign: 'center', marginBottom: '8px' }}>
            ...
            <h1 style={{ fontSize: '26px', fontWeight: '700', color: THEME.primary, margin: '0 0 4px' }}>PT Manager</h1>
            <p style={{ fontSize: '13px', color: THEME.textSub, margin: 0 }}>인천 체형교정 트레이닝</p>
          </div>
      ...
      {mode === 'trainer' && (
        <div style={S.loginCard}>
          <h2 style={{ fontSize: '20px', fontWeight: '700', color: THEME.text, textAlign: 'center', margin: '0 0 16px' }}>👨‍💼 트레이너 로그인</h2>
          {error && <p style={{ color: THEME.danger, fontSize: '13px', textAlign: 'center', margin: '0 0 8px' }}>{error}</p>}
      ...
      {mode === 'member' && (
        <div style={S.loginCard}>
          <h2 style={{ fontSize: '20px', fontWeight: '700', color: THEME.text, textAlign: 'center', margin: '0 0 4px' }}>🙋 회원 접속</h2>
          <p style={{ fontSize: '13px', color: THEME.textSub, textAlign: 'center', margin: '0 0 16px' }}>트레이너에게 받은 코드를 입력해주세요</p>
          {error && <p style={{ color: THEME.danger, fontSize: '13px', textAlign: 'center', margin: '0 0 8px' }}>{error}</p>}
